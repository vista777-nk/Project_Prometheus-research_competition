...
PX4 原生 Iris 模型已有 IMU/GPS/气压计（通过 Gazebo-PX4 桥接发布到 MAVROS）。
我们只需扩展一个深度相机 sensor，直接嵌入到机体的 `base_link` 中，无需独立静态模型。
...
<sdf version="1.5">
  <model name="iris_depth_camera">
    ...
    <!-- 复用 PX4 Iris 核心（含 IMU/GPS/气压计/电机） -->
    {% include 'iris/iris_base.sdf.jinja' %}

    <!--
      深度相机插件 (Intel RealSense D435i 模拟)
      直接附加在机体 base_link 上，前向安装，向下倾斜 45°。
      ⚠ sensor 标签必须位于 iris_base 中已有 link 的 scope 内，
      此处通过 joint 方式嵌入：为相机创建轻量 link，再用 fixed joint 刚性绑定到 base_link。
    -->
    <link name="depth_camera_link">
      <pose>0.15 0 0.03 0 0.785 0</pose>  <!-- 前向 15cm，低头 45° -->
      <inertial>
    ...
    <!--
      ...
      - IMU:   /mavros/imu/data
      参见 §2.10 的 gps_converter.py 做本地 ENU 转换。
    -->
...
删除独立模型创建步骤（不再需要）：
```bash
# 不再需要 ~/.gazebo/models/depth_camera/ 和 ~/.gazebo/models/gps/
...
## 2.5 创建 drone_bringup Launch 文件
...
**统一使用** `car_bringup` 的 `empty.world`（Task-03 创建），drone 不单独维护一份：
```bash
# 不需要独立创建 drone_bringup/worlds/，引用 car_bringup 的即可
# launch 文件中使用 $(find car_bringup)/worlds/empty.world
```
...
## 2.11 更新 `drone_bringup/CMakeLists.txt`
...
1. 先启动一个 Gazebo（通过 `car_bringup` 或独立 launch）
