...
1. **边缘�?*：无人机和车机上各运行一�?preprocessor 节点，聚合传感器数据�?`SensorFusion` 消息
2. **服务器端**：TCP 接收服务�?+ SLAM/EQA/协调器占位节�?
...
class CarPreprocessor:
    ...
    def _publish(self, event):
        ...
        # 超声波默认�?4.0m (max range)，表示无遮挡。不能用 0.0 否则避障会认为四面是墙�?
