...
- [四、Convention（编码与文档规范）](#四convention编码与文档规范)
...
main ────────────────────────────── ● (稳定，永远可部署)
     \
      feat/task-01-env-setup ──●──● (开发完成后合并回 main，分支删除)
```

| 分支类型 | 命名格式 | 用途 | 生命周期 |
|----------|----------|------|:---:|
| `main` | — | 稳定主线，永远可运行 | 永久 |
| `feat/<description>` | `feat/task-03-diff-chassis` | 新功能 / 新任务 | 合并后删除 |
| `fix/<description>` | `fix/icd-typo` | Bug 修复 | 合并后删除 |
| `docs/<description>` | `docs/adr-0002` | 纯文档变更 | 合并后删除 |
| `exp/<description>` | `exp/vlm-baseline-test` | 实验性分支（可能不合入） | 视情况保留或删除 |

**铁律**：
- `main` 分支永远不直接推送。所有变更通过分支 + 合并进入。
- 合并前确认 `main` 上的代码可以编译 / 仿真可以启动（至少冒烟测试通过）。
- 实验分支 (`exp/`) 可以不合入 `main`，但**不能删除**——保留为实验记录。
...
| Type | 用途 | 示例 |
|------|------|------|
| `feat` | 新功能 | `feat(task-02): 添加 PX4 SITL 无人机 Gazebo 模型` |
...
| `style` | 格式调整 | `style: PEP 8 格式化所有 Python 文件` |
...
| `milestone/<name>` | 里程碑节点 | `milestone/sim-framework-done` |
...
| 规则 | 示例 | 说明 |
|------|------|------|
| **顶层目录用 kebab-case** | `project-docs/`, `obsolete-docs/` | 全小写 + 连字符 |
| **Git 仓库内的 ROS package 目录例外** | `air_ground_interfaces/` | 用 snake_case，遵循 ROS 惯例 |
| **不以数字开头** | ❌ `1-docs/` → ✅ `docs/` | |
...
│   ├── air_ground_lab_server/             # 边缘预处理 + 服务端
│   └── ...
├── build/
├── devel/
└── setup_all.sh
...
| `air_ground_drone_bringup` | Layer 1 | 无人机仿真 |
| `air_ground_car_bringup` | Layer 1 | 车机仿真 |
| `air_ground_com_bridge` | Layer 2 | 空地/车服通信桥 |
| `air_ground_lab_server` | Layer 2~3 | 边缘预处理 + 服务端 |
| `air_ground_vlm` | Layer 4 | VLM 研究模块 |
| `air_ground_slam` | Layer 4 | SLAM 研究模块 |
| `air_ground_planner` | Layer 4 | Planner 研究模块 |

**铁律**：
- 所有 package 以 `air_ground_` 为前缀，避免与 ROS 官方包或第三方包冲突。
- Layer 4 package 永远不依赖 `mavros`、`gazebo_msgs`、`sensor_msgs` 等硬件相关包。
...
| 模式 | 示例 |
|------|------|
| 传感器数据 | `/drone/rgb/image_raw` |
| 抽象观测 | `/drone/observation` |
| 机器人状态 | `/car/robot_state` |
| 控制指令 | `/car/cmd_vel` |
| 世界状态 | `/world_state` |
| 任务/使命 | `/mission` |
...
**铁律**：
- Topic 名用 `snake_case`。
- 以 `/robot_id` 为第一级命名空间（`drone` / `car`）。
- 全局 Topic（如 `/world_state`, `/mission`）不加 robot_id 前缀。
- 抽象接口层 (Layer 3) 的 Topic 命名遵循 ICD.md。
...
| `car_sensor_fusion` | 车机传感器融合节点 |
| `mavlink_bridge` | MAVLink 协议翻译节点 |
| `tcp_bridge` | TCP 通信桥节点 |
| `world_model` | 世界模型节点 |
...
| 元素 | 格式 | 示例 |
|------|------|------|
| Python 类 | `PascalCase` | `WorldModel`, `ObservationAggregator` |
...
| C++ 类 | `PascalCase` | `MecanumController` |
| C++ 函数 | `camelCase` 或 `snake_case` | （与 ROS 现有代码风格一致） |
...
- 严格遵循 **PEP 8**。
- 使用 **4 空格缩进**（不用 Tab）。
- 最大行宽 **100 字符**（ROS 惯例，比标准 PEP 8 的 79 略宽）。
- 所有公共函数/类 **必须写 docstring**（Google Style 或 NumPy Style）。
- 鼓励使用 **Type Hints**（Python 3.8+ 支持）。

```python
"""传感器观测聚合节点。

将多个传感器 Topic 聚合为统一的 Observation 消息，
按 ICD v1.0 规范发布到 /<robot_id>/observation。

Note:
    本节点属于 Layer 2 (Bridge)，负责协议翻译。
...
class ObservationAggregator:
    ...
    Attributes:
        robot_id: 机器人标识符 ("drone" 或 "car")。
        max_frequency_hz: 最大发布频率 (Hz)。
    """
    ...
    def process_observation(self, modalities: List[str]) -> Optional[Observation]:
        ...
        Args:
            modalities: 本次携带的模态列表，如 ["rgb", "depth", "imu"]。
...
| 内容 | 语言 | 理由 |
|------|:---:|------|
| 变量名、函数名、类名 | **English** | 编程语言惯例，ROS 生态兼容 |
| 模块/类 docstring | **中文** | 项目团队母语，降低理解门槛 |
| 行内注释 | **中文** | 同上 |
| Commit message | **中文** | 同上 |
| Git 分支名 | **English** | 工具链兼容性 |
| Markdown 文档 | **中文** | 同上 |
...
- [ ] 新 package 遵循 `air_ground_` 命名前缀
- [ ] Layer 4 代码没有 import 硬件相关包
- [ ] 参数全部从 YAML 读取，无硬编码
- [ ] 公共函数/类有 docstring
...
| 场景 | 查这里 |
|------|--------|
| 新建一个 Git 分支 | [§1.1 分支策略](#11-分支策略) |
| 写一条 commit message | [§1.2 Commit Message 规范](#12-commit-message-规范) |
| 新建一个文件/目录 | [§2.2 目录命名约定](#22-目录命名约定) + [§3.1 文件命名](#31-文件命名) |
| 新建一个 ROS package | [§3.2 ROS 命名 → Package](#package) |
| 新建一个 ROS topic | [§3.2 ROS 命名 → Topics](#topics) |
| 写一个 Python 类/函数 | [§3.3 代码命名](#33-代码命名) + [§4.1 Python 编码规范](#41-python-编码规范) |
| 合并代码到 main 前 | [§4.6 代码审查清单](#46-代码审查清单) |
