...
│   ├── bsp.c/.h                  时钟树 168MHz、SysTick、GPIO、急停、LED
│   ├── encoder.c/.h              TIM2/3/4/5 编码器模式
│   ├── motor.c/.h                TIM1 PWM + 方向脚 + ADC 电流
│   ├── uart.c/.h                 USART1 中断收发 + 环形缓冲
...
|---------------------------|------------------|
| 硬件急停检测 · 编码器采样 · 逆运动学 · 四路 PID · PWM 输出 · 堵转/超时检测 | 串口拆帧与命令分发 · 空闲重同步 · ADC 电流采样 · 20Hz 遥测 · 状态灯 |
...
> **本固件的 HAL 层（`bsp.c` / `encoder.c` / `motor.c` / `uart.c`）尚未在真实硬件上验证。**
> Phase 1 的目标是"CI 编译通过 + Host 单元测试通过"。首次上板请按下表逐项确认。
