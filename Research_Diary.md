###### 2026/7/25
    发现：ChatGPT 终审指出项目缺少"设计哲学"层——只有 Platform/Roadmap/ICD/Tasks，没有"为什么这样设计"。
    ...
    完成：1) RESEARCH_PHILOSOPHY.md —— 项目宪法文档，含五条原则 + ADR 规范 + 长期愿景 + 角色平衡
    ...
    失败：无
    小结：规划阶段正式完结。项目现在拥有完整的四份核心文献：哲学（why）→ 架构（what）→ 契约（how to connect）→ 路线（where to go），
          加上一套可执行的基础设施规范（CONVENTIONS.md）。ChatGPT 终审意见全部落地。可以正式开始仿真搭建。
    下一步：task-01-env-setup.md —— 在 Ubuntu 20.04 上搭建 ROS Noetic + Gazebo 9 + PX4 工具链 + 工作空间脚手架