...
static void Write_8bit_TFT18(unsigned char dat);
static void Write_16bit_TFT18(unsigned short dat);
...
void Init_TFT18(void)
{
    // SPI初始化
#if TFT_HARD_SPI
    init_hard_spi(TFT_PIN, TFT_SPPED, 0, 0);
#endif
    ...
    {
    case 0:
        Write_8bit_TFT18(0xC0);
        break;
    case 1:
        Write_8bit_TFT18(0X00);
        break;
    case 2:
        Write_8bit_TFT18(0xA0);
        break;
    case 3:
        Write_8bit_TFT18(0x60);
        break;
    }
...
* Example:  Show_Chinese16x16_TFT(0,0,Chinese16x16[0],0,3,BLACK,WHITE,16);    //在左上角显示IPSChinese16x16数组中的第0~3个汉字即“呆萌侠”
*
**/
void Show_Chinese16x16_TFT(unsigned int x, unsigned int y, const unsigned char *dat, unsigned char startnumber, unsigned char number, COLOR_enum fc, COLOR_enum bc, unsigned char fsize)
{
    unsigned char i, j, k;
    unsigned char temp;
    const unsigned char *pdat;

    Set_Pos_TFT18(x, y, (number - startnumber) * fsize - 1 + x, y + fsize - 1);
...
void Show_Picture_TFT(unsigned int x, unsigned int y, unsigned int width, unsigned int heigh, const unsigned char pic[])
{
    ...
    for(i = 0; i < width; i++)
    {
        for(j = 0; j < heigh; j++)
        {
            Write_8bit_TFT18(pic[k * 2]);
            Write_8bit_TFT18(pic[k * 2 + 1]);
            k++;
        }
    }
}
