...
void Init_OLED(void)
{
    ...
    Write_Commond_OLED(0x40);   //--set start line address  Set Mapping RAM Display Start Line (0x00~0x3F)
    Write_Commond_OLED(0x81);   //--set contrast control register
    Write_Commond_OLED(0xcf);   // Set SEG Output Current Brightness
    Write_Commond_OLED(0xa1);   //--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
    Write_Commond_OLED(0xc8);   //Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
    Write_Commond_OLED(0xa6);   //--set normal display
    Write_Commond_OLED(0xa8);   //--set multiplex ratio(1 to 64)
    Write_Commond_OLED(0x3f);   //--1/64 duty
    Write_Commond_OLED(0xd3);   //-set display offset  Shift Mapping RAM Counter (0x00~0x3F)
    Write_Commond_OLED(0x00);   //-not offset
    Write_Commond_OLED(0xd5);   //--set display clock divide ratio/oscillator frequency
    Write_Commond_OLED(0x80);   //--set divide ratio, Set Clock as 100 Frames/Sec
    Write_Commond_OLED(0xd9);   //--set pre-charge period
    Write_Commond_OLED(0xf1);   //Set Pre-Charge as 15 Clocks & Discharge as 1 Clock
    Write_Commond_OLED(0xda);   //--set com pins hardware configuration
    Write_Commond_OLED(0x12);
    Write_Commond_OLED(0xdb);   //--set vcomh
    Write_Commond_OLED(0x40);   //Set VCOM Deselect Level
    Write_Commond_OLED(0x20);   //-Set Page Addressing Mode (0x00/0x01/0x02)
    Write_Commond_OLED(0x02);
    Write_Commond_OLED(0x8d);   //--set Charge Pump enable/disable
    Write_Commond_OLED(0x14);   //--set(0x10) disable
    Write_Commond_OLED(0xa4);   // Disable Entire Display On (0xa4/0xa5)
    Write_Commond_OLED(0xa6);   // Disable Inverse Display On (0xa6/a7)
    Write_Commond_OLED(0xaf);   //--turn on oled panel
    Clean_OLED();         //初始清屏
    Set_Pos_OLED(0, 0);
}
