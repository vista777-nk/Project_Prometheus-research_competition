...
void LCD_X_Config(void) {
	...
	}else if(lcddev.id == 0X5310 || lcddev.id == 0X6804) //0X5510 0X6804为3.5寸 320x480
	{
		...
		{
		LCD_SetSizeEx    (0, lcddev.width, lcddev.height);//设置可见区域尺寸
		LCD_SetVSizeEx   (0, lcddev.width, lcddev.height);//设置虚拟区域尺寸
		
		if(lcddev.dir == 0) //竖屏
		{
			GUI_TOUCH_SetOrientation(GUI_MIRROR_X);//设置为竖屏 
			GUI_TOUCH_Calibrate(GUI_COORD_X,0,lcddev.width,155,3903);
			GUI_TOUCH_Calibrate(GUI_COORD_Y,0,lcddev.height,188,3935);
		}else //横屏
		{
			GUI_TOUCH_SetOrientation(GUI_SWAP_XY|GUI_MIRROR_X|GUI_MIRROR_Y); //设置为横屏
